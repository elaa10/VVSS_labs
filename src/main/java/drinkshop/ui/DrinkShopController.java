...
public class DrinkShopController {
    ...
    private void onAddNewIngred() {
        try {
            // Parse ingredient details and add to the temporary recipe list
            String name = txtNewIngredName.getText();
            double quantity = Double.parseDouble(txtNewIngredCant.getText());
            newRetetaList.add(new IngredientReteta(name, quantity));
        } catch (NumberFormatException e) {
            // Display an alert if the user leaves the quantity field blank or enters text
